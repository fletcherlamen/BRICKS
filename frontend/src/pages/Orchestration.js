          ...
          >
            ...
            {executionResults ? (
              <div className="space-y-6 max-h-screen overflow-y-auto">
                    ...
                    <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 max-h-96 overflow-y-auto">
                      ...
                      <div className="mt-2 text-xs text-blue-600 flex items-center justify-between">
                        ...
                        <div className="text-blue-500">
                          {executionResults.results.gpt4_analysis.response?.length || 0} characters
                        </div>
                      </div>
                    </div>
                  </div>
                ...
                {executionResults.results?.revenue_opportunities && (
                  <div className="mb-6">
                    <h4 className="text-sm font-medium text-gray-900 mb-3 flex items-center">
                      <ExclamationTriangleIcon className="h-4 w-4 mr-2 text-green-500" />
                      Revenue Analysis:
                    </h4>
                    <div className="bg-green-50 border border-green-200 rounded-lg p-4">
                      <div className="text-sm text-gray-800">
                        <p><strong>Status:</strong> {executionResults.results.revenue_opportunities.status}</p>
                        <p><strong>Recommendation:</strong> {executionResults.results.revenue_opportunities.recommendation}</p>
                        <p><strong>Total Opportunities:</strong> {executionResults.results.revenue_opportunities.total_opportunities}</p>
                      </div>
                    </div>
                  </div>
                )}
                ...
                {/* Complete Results Display - No Truncation */}
                {(executionResults.results?.gpt4_analysis?.response || executionResults.results?.analysis?.business?.revenue_analysis) && (
                  <div className="mt-6 p-4 bg-gradient-to-r from-blue-50 to-indigo-50 border border-blue-300 rounded-lg">
                    <div className="flex items-center justify-between mb-3">
                      <h4 className="text-lg font-semibold text-gray-900 flex items-center">
                        ...
                        Complete AI Analysis - Full Response
                      </h4>
                      <div className="text-xs text-blue-600 bg-blue-100 px-2 py-1 rounded">
                        {(executionResults.results.gpt4_analysis?.response || 
                          executionResults.results.analysis?.business?.revenue_analysis || '').length} characters
                      </div>
                    </div>
                    <div className="bg-white border border-gray-200 rounded-lg p-6 max-h-screen overflow-y-auto shadow-inner">
                      <div className="prose prose-sm max-w-none">
                        <pre className="text-sm text-gray-800 whitespace-pre-wrap font-sans leading-relaxed m-0">
                          {executionResults.results.gpt4_analysis?.response || 
                    ...
                    <div className="mt-3 flex items-center justify-between text-xs text-gray-600">
                      <div className="flex items-center">
                        <CpuChipIcon className="h-3 w-3 mr-1 text-blue-500" />
                        Generated by: {executionResults.results.gpt4_analysis.model_used || 'GPT-4'}
                      </div>
